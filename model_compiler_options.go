/*
Alephium API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 4.5.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package alephium

import (
	"encoding/json"
)

// checks if the CompilerOptions type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CompilerOptions{}

// CompilerOptions struct for CompilerOptions
type CompilerOptions struct {
	IgnoreUnusedConstantsWarnings *bool `json:"ignoreUnusedConstantsWarnings,omitempty"`
	IgnoreUnusedVariablesWarnings *bool `json:"ignoreUnusedVariablesWarnings,omitempty"`
	IgnoreUnusedFieldsWarnings *bool `json:"ignoreUnusedFieldsWarnings,omitempty"`
	IgnoreUnusedPrivateFunctionsWarnings *bool `json:"ignoreUnusedPrivateFunctionsWarnings,omitempty"`
	IgnoreUpdateFieldsCheckWarnings *bool `json:"ignoreUpdateFieldsCheckWarnings,omitempty"`
	IgnoreCheckExternalCallerWarnings *bool `json:"ignoreCheckExternalCallerWarnings,omitempty"`
	IgnoreUnusedFunctionReturnWarnings *bool `json:"ignoreUnusedFunctionReturnWarnings,omitempty"`
	SkipAbstractContractCheck *bool `json:"skipAbstractContractCheck,omitempty"`
	SkipTests *bool `json:"skipTests,omitempty"`
}

// NewCompilerOptions instantiates a new CompilerOptions object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCompilerOptions() *CompilerOptions {
	this := CompilerOptions{}
	return &this
}

// NewCompilerOptionsWithDefaults instantiates a new CompilerOptions object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCompilerOptionsWithDefaults() *CompilerOptions {
	this := CompilerOptions{}
	return &this
}

// GetIgnoreUnusedConstantsWarnings returns the IgnoreUnusedConstantsWarnings field value if set, zero value otherwise.
func (o *CompilerOptions) GetIgnoreUnusedConstantsWarnings() bool {
	if o == nil || IsNil(o.IgnoreUnusedConstantsWarnings) {
		var ret bool
		return ret
	}
	return *o.IgnoreUnusedConstantsWarnings
}

// GetIgnoreUnusedConstantsWarningsOk returns a tuple with the IgnoreUnusedConstantsWarnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetIgnoreUnusedConstantsWarningsOk() (*bool, bool) {
	if o == nil || IsNil(o.IgnoreUnusedConstantsWarnings) {
		return nil, false
	}
	return o.IgnoreUnusedConstantsWarnings, true
}

// HasIgnoreUnusedConstantsWarnings returns a boolean if a field has been set.
func (o *CompilerOptions) HasIgnoreUnusedConstantsWarnings() bool {
	if o != nil && !IsNil(o.IgnoreUnusedConstantsWarnings) {
		return true
	}

	return false
}

// SetIgnoreUnusedConstantsWarnings gets a reference to the given bool and assigns it to the IgnoreUnusedConstantsWarnings field.
func (o *CompilerOptions) SetIgnoreUnusedConstantsWarnings(v bool) {
	o.IgnoreUnusedConstantsWarnings = &v
}

// GetIgnoreUnusedVariablesWarnings returns the IgnoreUnusedVariablesWarnings field value if set, zero value otherwise.
func (o *CompilerOptions) GetIgnoreUnusedVariablesWarnings() bool {
	if o == nil || IsNil(o.IgnoreUnusedVariablesWarnings) {
		var ret bool
		return ret
	}
	return *o.IgnoreUnusedVariablesWarnings
}

// GetIgnoreUnusedVariablesWarningsOk returns a tuple with the IgnoreUnusedVariablesWarnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetIgnoreUnusedVariablesWarningsOk() (*bool, bool) {
	if o == nil || IsNil(o.IgnoreUnusedVariablesWarnings) {
		return nil, false
	}
	return o.IgnoreUnusedVariablesWarnings, true
}

// HasIgnoreUnusedVariablesWarnings returns a boolean if a field has been set.
func (o *CompilerOptions) HasIgnoreUnusedVariablesWarnings() bool {
	if o != nil && !IsNil(o.IgnoreUnusedVariablesWarnings) {
		return true
	}

	return false
}

// SetIgnoreUnusedVariablesWarnings gets a reference to the given bool and assigns it to the IgnoreUnusedVariablesWarnings field.
func (o *CompilerOptions) SetIgnoreUnusedVariablesWarnings(v bool) {
	o.IgnoreUnusedVariablesWarnings = &v
}

// GetIgnoreUnusedFieldsWarnings returns the IgnoreUnusedFieldsWarnings field value if set, zero value otherwise.
func (o *CompilerOptions) GetIgnoreUnusedFieldsWarnings() bool {
	if o == nil || IsNil(o.IgnoreUnusedFieldsWarnings) {
		var ret bool
		return ret
	}
	return *o.IgnoreUnusedFieldsWarnings
}

// GetIgnoreUnusedFieldsWarningsOk returns a tuple with the IgnoreUnusedFieldsWarnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetIgnoreUnusedFieldsWarningsOk() (*bool, bool) {
	if o == nil || IsNil(o.IgnoreUnusedFieldsWarnings) {
		return nil, false
	}
	return o.IgnoreUnusedFieldsWarnings, true
}

// HasIgnoreUnusedFieldsWarnings returns a boolean if a field has been set.
func (o *CompilerOptions) HasIgnoreUnusedFieldsWarnings() bool {
	if o != nil && !IsNil(o.IgnoreUnusedFieldsWarnings) {
		return true
	}

	return false
}

// SetIgnoreUnusedFieldsWarnings gets a reference to the given bool and assigns it to the IgnoreUnusedFieldsWarnings field.
func (o *CompilerOptions) SetIgnoreUnusedFieldsWarnings(v bool) {
	o.IgnoreUnusedFieldsWarnings = &v
}

// GetIgnoreUnusedPrivateFunctionsWarnings returns the IgnoreUnusedPrivateFunctionsWarnings field value if set, zero value otherwise.
func (o *CompilerOptions) GetIgnoreUnusedPrivateFunctionsWarnings() bool {
	if o == nil || IsNil(o.IgnoreUnusedPrivateFunctionsWarnings) {
		var ret bool
		return ret
	}
	return *o.IgnoreUnusedPrivateFunctionsWarnings
}

// GetIgnoreUnusedPrivateFunctionsWarningsOk returns a tuple with the IgnoreUnusedPrivateFunctionsWarnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetIgnoreUnusedPrivateFunctionsWarningsOk() (*bool, bool) {
	if o == nil || IsNil(o.IgnoreUnusedPrivateFunctionsWarnings) {
		return nil, false
	}
	return o.IgnoreUnusedPrivateFunctionsWarnings, true
}

// HasIgnoreUnusedPrivateFunctionsWarnings returns a boolean if a field has been set.
func (o *CompilerOptions) HasIgnoreUnusedPrivateFunctionsWarnings() bool {
	if o != nil && !IsNil(o.IgnoreUnusedPrivateFunctionsWarnings) {
		return true
	}

	return false
}

// SetIgnoreUnusedPrivateFunctionsWarnings gets a reference to the given bool and assigns it to the IgnoreUnusedPrivateFunctionsWarnings field.
func (o *CompilerOptions) SetIgnoreUnusedPrivateFunctionsWarnings(v bool) {
	o.IgnoreUnusedPrivateFunctionsWarnings = &v
}

// GetIgnoreUpdateFieldsCheckWarnings returns the IgnoreUpdateFieldsCheckWarnings field value if set, zero value otherwise.
func (o *CompilerOptions) GetIgnoreUpdateFieldsCheckWarnings() bool {
	if o == nil || IsNil(o.IgnoreUpdateFieldsCheckWarnings) {
		var ret bool
		return ret
	}
	return *o.IgnoreUpdateFieldsCheckWarnings
}

// GetIgnoreUpdateFieldsCheckWarningsOk returns a tuple with the IgnoreUpdateFieldsCheckWarnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetIgnoreUpdateFieldsCheckWarningsOk() (*bool, bool) {
	if o == nil || IsNil(o.IgnoreUpdateFieldsCheckWarnings) {
		return nil, false
	}
	return o.IgnoreUpdateFieldsCheckWarnings, true
}

// HasIgnoreUpdateFieldsCheckWarnings returns a boolean if a field has been set.
func (o *CompilerOptions) HasIgnoreUpdateFieldsCheckWarnings() bool {
	if o != nil && !IsNil(o.IgnoreUpdateFieldsCheckWarnings) {
		return true
	}

	return false
}

// SetIgnoreUpdateFieldsCheckWarnings gets a reference to the given bool and assigns it to the IgnoreUpdateFieldsCheckWarnings field.
func (o *CompilerOptions) SetIgnoreUpdateFieldsCheckWarnings(v bool) {
	o.IgnoreUpdateFieldsCheckWarnings = &v
}

// GetIgnoreCheckExternalCallerWarnings returns the IgnoreCheckExternalCallerWarnings field value if set, zero value otherwise.
func (o *CompilerOptions) GetIgnoreCheckExternalCallerWarnings() bool {
	if o == nil || IsNil(o.IgnoreCheckExternalCallerWarnings) {
		var ret bool
		return ret
	}
	return *o.IgnoreCheckExternalCallerWarnings
}

// GetIgnoreCheckExternalCallerWarningsOk returns a tuple with the IgnoreCheckExternalCallerWarnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetIgnoreCheckExternalCallerWarningsOk() (*bool, bool) {
	if o == nil || IsNil(o.IgnoreCheckExternalCallerWarnings) {
		return nil, false
	}
	return o.IgnoreCheckExternalCallerWarnings, true
}

// HasIgnoreCheckExternalCallerWarnings returns a boolean if a field has been set.
func (o *CompilerOptions) HasIgnoreCheckExternalCallerWarnings() bool {
	if o != nil && !IsNil(o.IgnoreCheckExternalCallerWarnings) {
		return true
	}

	return false
}

// SetIgnoreCheckExternalCallerWarnings gets a reference to the given bool and assigns it to the IgnoreCheckExternalCallerWarnings field.
func (o *CompilerOptions) SetIgnoreCheckExternalCallerWarnings(v bool) {
	o.IgnoreCheckExternalCallerWarnings = &v
}

// GetIgnoreUnusedFunctionReturnWarnings returns the IgnoreUnusedFunctionReturnWarnings field value if set, zero value otherwise.
func (o *CompilerOptions) GetIgnoreUnusedFunctionReturnWarnings() bool {
	if o == nil || IsNil(o.IgnoreUnusedFunctionReturnWarnings) {
		var ret bool
		return ret
	}
	return *o.IgnoreUnusedFunctionReturnWarnings
}

// GetIgnoreUnusedFunctionReturnWarningsOk returns a tuple with the IgnoreUnusedFunctionReturnWarnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetIgnoreUnusedFunctionReturnWarningsOk() (*bool, bool) {
	if o == nil || IsNil(o.IgnoreUnusedFunctionReturnWarnings) {
		return nil, false
	}
	return o.IgnoreUnusedFunctionReturnWarnings, true
}

// HasIgnoreUnusedFunctionReturnWarnings returns a boolean if a field has been set.
func (o *CompilerOptions) HasIgnoreUnusedFunctionReturnWarnings() bool {
	if o != nil && !IsNil(o.IgnoreUnusedFunctionReturnWarnings) {
		return true
	}

	return false
}

// SetIgnoreUnusedFunctionReturnWarnings gets a reference to the given bool and assigns it to the IgnoreUnusedFunctionReturnWarnings field.
func (o *CompilerOptions) SetIgnoreUnusedFunctionReturnWarnings(v bool) {
	o.IgnoreUnusedFunctionReturnWarnings = &v
}

// GetSkipAbstractContractCheck returns the SkipAbstractContractCheck field value if set, zero value otherwise.
func (o *CompilerOptions) GetSkipAbstractContractCheck() bool {
	if o == nil || IsNil(o.SkipAbstractContractCheck) {
		var ret bool
		return ret
	}
	return *o.SkipAbstractContractCheck
}

// GetSkipAbstractContractCheckOk returns a tuple with the SkipAbstractContractCheck field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetSkipAbstractContractCheckOk() (*bool, bool) {
	if o == nil || IsNil(o.SkipAbstractContractCheck) {
		return nil, false
	}
	return o.SkipAbstractContractCheck, true
}

// HasSkipAbstractContractCheck returns a boolean if a field has been set.
func (o *CompilerOptions) HasSkipAbstractContractCheck() bool {
	if o != nil && !IsNil(o.SkipAbstractContractCheck) {
		return true
	}

	return false
}

// SetSkipAbstractContractCheck gets a reference to the given bool and assigns it to the SkipAbstractContractCheck field.
func (o *CompilerOptions) SetSkipAbstractContractCheck(v bool) {
	o.SkipAbstractContractCheck = &v
}

// GetSkipTests returns the SkipTests field value if set, zero value otherwise.
func (o *CompilerOptions) GetSkipTests() bool {
	if o == nil || IsNil(o.SkipTests) {
		var ret bool
		return ret
	}
	return *o.SkipTests
}

// GetSkipTestsOk returns a tuple with the SkipTests field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CompilerOptions) GetSkipTestsOk() (*bool, bool) {
	if o == nil || IsNil(o.SkipTests) {
		return nil, false
	}
	return o.SkipTests, true
}

// HasSkipTests returns a boolean if a field has been set.
func (o *CompilerOptions) HasSkipTests() bool {
	if o != nil && !IsNil(o.SkipTests) {
		return true
	}

	return false
}

// SetSkipTests gets a reference to the given bool and assigns it to the SkipTests field.
func (o *CompilerOptions) SetSkipTests(v bool) {
	o.SkipTests = &v
}

func (o CompilerOptions) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CompilerOptions) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.IgnoreUnusedConstantsWarnings) {
		toSerialize["ignoreUnusedConstantsWarnings"] = o.IgnoreUnusedConstantsWarnings
	}
	if !IsNil(o.IgnoreUnusedVariablesWarnings) {
		toSerialize["ignoreUnusedVariablesWarnings"] = o.IgnoreUnusedVariablesWarnings
	}
	if !IsNil(o.IgnoreUnusedFieldsWarnings) {
		toSerialize["ignoreUnusedFieldsWarnings"] = o.IgnoreUnusedFieldsWarnings
	}
	if !IsNil(o.IgnoreUnusedPrivateFunctionsWarnings) {
		toSerialize["ignoreUnusedPrivateFunctionsWarnings"] = o.IgnoreUnusedPrivateFunctionsWarnings
	}
	if !IsNil(o.IgnoreUpdateFieldsCheckWarnings) {
		toSerialize["ignoreUpdateFieldsCheckWarnings"] = o.IgnoreUpdateFieldsCheckWarnings
	}
	if !IsNil(o.IgnoreCheckExternalCallerWarnings) {
		toSerialize["ignoreCheckExternalCallerWarnings"] = o.IgnoreCheckExternalCallerWarnings
	}
	if !IsNil(o.IgnoreUnusedFunctionReturnWarnings) {
		toSerialize["ignoreUnusedFunctionReturnWarnings"] = o.IgnoreUnusedFunctionReturnWarnings
	}
	if !IsNil(o.SkipAbstractContractCheck) {
		toSerialize["skipAbstractContractCheck"] = o.SkipAbstractContractCheck
	}
	if !IsNil(o.SkipTests) {
		toSerialize["skipTests"] = o.SkipTests
	}
	return toSerialize, nil
}

type NullableCompilerOptions struct {
	value *CompilerOptions
	isSet bool
}

func (v NullableCompilerOptions) Get() *CompilerOptions {
	return v.value
}

func (v *NullableCompilerOptions) Set(val *CompilerOptions) {
	v.value = val
	v.isSet = true
}

func (v NullableCompilerOptions) IsSet() bool {
	return v.isSet
}

func (v *NullableCompilerOptions) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCompilerOptions(val *CompilerOptions) *NullableCompilerOptions {
	return &NullableCompilerOptions{value: val, isSet: true}
}

func (v NullableCompilerOptions) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCompilerOptions) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


