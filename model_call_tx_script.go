/*
Alephium API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 4.7.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package alephium

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the CallTxScript type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CallTxScript{}

// CallTxScript struct for CallTxScript
type CallTxScript struct {
	Group int32 `json:"group"`
	Bytecode string `json:"bytecode"`
	CallerAddress *string `json:"callerAddress,omitempty"`
	WorldStateBlockHash *string `json:"worldStateBlockHash,omitempty"`
	TxId *string `json:"txId,omitempty"`
	InputAssets []TestInputAsset `json:"inputAssets,omitempty"`
	InterestedContracts []string `json:"interestedContracts,omitempty"`
}

type _CallTxScript CallTxScript

// NewCallTxScript instantiates a new CallTxScript object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCallTxScript(group int32, bytecode string) *CallTxScript {
	this := CallTxScript{}
	this.Group = group
	this.Bytecode = bytecode
	return &this
}

// NewCallTxScriptWithDefaults instantiates a new CallTxScript object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCallTxScriptWithDefaults() *CallTxScript {
	this := CallTxScript{}
	return &this
}

// GetGroup returns the Group field value
func (o *CallTxScript) GetGroup() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Group
}

// GetGroupOk returns a tuple with the Group field value
// and a boolean to check if the value has been set.
func (o *CallTxScript) GetGroupOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Group, true
}

// SetGroup sets field value
func (o *CallTxScript) SetGroup(v int32) {
	o.Group = v
}

// GetBytecode returns the Bytecode field value
func (o *CallTxScript) GetBytecode() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Bytecode
}

// GetBytecodeOk returns a tuple with the Bytecode field value
// and a boolean to check if the value has been set.
func (o *CallTxScript) GetBytecodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Bytecode, true
}

// SetBytecode sets field value
func (o *CallTxScript) SetBytecode(v string) {
	o.Bytecode = v
}

// GetCallerAddress returns the CallerAddress field value if set, zero value otherwise.
func (o *CallTxScript) GetCallerAddress() string {
	if o == nil || IsNil(o.CallerAddress) {
		var ret string
		return ret
	}
	return *o.CallerAddress
}

// GetCallerAddressOk returns a tuple with the CallerAddress field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallTxScript) GetCallerAddressOk() (*string, bool) {
	if o == nil || IsNil(o.CallerAddress) {
		return nil, false
	}
	return o.CallerAddress, true
}

// HasCallerAddress returns a boolean if a field has been set.
func (o *CallTxScript) HasCallerAddress() bool {
	if o != nil && !IsNil(o.CallerAddress) {
		return true
	}

	return false
}

// SetCallerAddress gets a reference to the given string and assigns it to the CallerAddress field.
func (o *CallTxScript) SetCallerAddress(v string) {
	o.CallerAddress = &v
}

// GetWorldStateBlockHash returns the WorldStateBlockHash field value if set, zero value otherwise.
func (o *CallTxScript) GetWorldStateBlockHash() string {
	if o == nil || IsNil(o.WorldStateBlockHash) {
		var ret string
		return ret
	}
	return *o.WorldStateBlockHash
}

// GetWorldStateBlockHashOk returns a tuple with the WorldStateBlockHash field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallTxScript) GetWorldStateBlockHashOk() (*string, bool) {
	if o == nil || IsNil(o.WorldStateBlockHash) {
		return nil, false
	}
	return o.WorldStateBlockHash, true
}

// HasWorldStateBlockHash returns a boolean if a field has been set.
func (o *CallTxScript) HasWorldStateBlockHash() bool {
	if o != nil && !IsNil(o.WorldStateBlockHash) {
		return true
	}

	return false
}

// SetWorldStateBlockHash gets a reference to the given string and assigns it to the WorldStateBlockHash field.
func (o *CallTxScript) SetWorldStateBlockHash(v string) {
	o.WorldStateBlockHash = &v
}

// GetTxId returns the TxId field value if set, zero value otherwise.
func (o *CallTxScript) GetTxId() string {
	if o == nil || IsNil(o.TxId) {
		var ret string
		return ret
	}
	return *o.TxId
}

// GetTxIdOk returns a tuple with the TxId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallTxScript) GetTxIdOk() (*string, bool) {
	if o == nil || IsNil(o.TxId) {
		return nil, false
	}
	return o.TxId, true
}

// HasTxId returns a boolean if a field has been set.
func (o *CallTxScript) HasTxId() bool {
	if o != nil && !IsNil(o.TxId) {
		return true
	}

	return false
}

// SetTxId gets a reference to the given string and assigns it to the TxId field.
func (o *CallTxScript) SetTxId(v string) {
	o.TxId = &v
}

// GetInputAssets returns the InputAssets field value if set, zero value otherwise.
func (o *CallTxScript) GetInputAssets() []TestInputAsset {
	if o == nil || IsNil(o.InputAssets) {
		var ret []TestInputAsset
		return ret
	}
	return o.InputAssets
}

// GetInputAssetsOk returns a tuple with the InputAssets field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallTxScript) GetInputAssetsOk() ([]TestInputAsset, bool) {
	if o == nil || IsNil(o.InputAssets) {
		return nil, false
	}
	return o.InputAssets, true
}

// HasInputAssets returns a boolean if a field has been set.
func (o *CallTxScript) HasInputAssets() bool {
	if o != nil && !IsNil(o.InputAssets) {
		return true
	}

	return false
}

// SetInputAssets gets a reference to the given []TestInputAsset and assigns it to the InputAssets field.
func (o *CallTxScript) SetInputAssets(v []TestInputAsset) {
	o.InputAssets = v
}

// GetInterestedContracts returns the InterestedContracts field value if set, zero value otherwise.
func (o *CallTxScript) GetInterestedContracts() []string {
	if o == nil || IsNil(o.InterestedContracts) {
		var ret []string
		return ret
	}
	return o.InterestedContracts
}

// GetInterestedContractsOk returns a tuple with the InterestedContracts field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CallTxScript) GetInterestedContractsOk() ([]string, bool) {
	if o == nil || IsNil(o.InterestedContracts) {
		return nil, false
	}
	return o.InterestedContracts, true
}

// HasInterestedContracts returns a boolean if a field has been set.
func (o *CallTxScript) HasInterestedContracts() bool {
	if o != nil && !IsNil(o.InterestedContracts) {
		return true
	}

	return false
}

// SetInterestedContracts gets a reference to the given []string and assigns it to the InterestedContracts field.
func (o *CallTxScript) SetInterestedContracts(v []string) {
	o.InterestedContracts = v
}

func (o CallTxScript) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CallTxScript) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["group"] = o.Group
	toSerialize["bytecode"] = o.Bytecode
	if !IsNil(o.CallerAddress) {
		toSerialize["callerAddress"] = o.CallerAddress
	}
	if !IsNil(o.WorldStateBlockHash) {
		toSerialize["worldStateBlockHash"] = o.WorldStateBlockHash
	}
	if !IsNil(o.TxId) {
		toSerialize["txId"] = o.TxId
	}
	if !IsNil(o.InputAssets) {
		toSerialize["inputAssets"] = o.InputAssets
	}
	if !IsNil(o.InterestedContracts) {
		toSerialize["interestedContracts"] = o.InterestedContracts
	}
	return toSerialize, nil
}

func (o *CallTxScript) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"group",
		"bytecode",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varCallTxScript := _CallTxScript{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varCallTxScript)

	if err != nil {
		return err
	}

	*o = CallTxScript(varCallTxScript)

	return err
}

type NullableCallTxScript struct {
	value *CallTxScript
	isSet bool
}

func (v NullableCallTxScript) Get() *CallTxScript {
	return v.value
}

func (v *NullableCallTxScript) Set(val *CallTxScript) {
	v.value = val
	v.isSet = true
}

func (v NullableCallTxScript) IsSet() bool {
	return v.isSet
}

func (v *NullableCallTxScript) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCallTxScript(val *CallTxScript) *NullableCallTxScript {
	return &NullableCallTxScript{value: val, isSet: true}
}

func (v NullableCallTxScript) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCallTxScript) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


